...
export function SearchModal({ sessions, open, onClose, onOpenSession }: {
  ...
  return (
    <div
      ...
      onMouseDown={e => { if (!panelRef.current?.contains(e.target as Node)) handleClose() }}
    >
      {/* Backdrop */}
      <div className={`absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-[180ms] ${visible ? 'opacity-100' : 'opacity-0'}`} />

      {/* Panel */}
      <div
        ref={panelRef}
        className={`relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 flex flex-col overflow-hidden transition-all duration-[180ms] ${visible ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2'}`}
            ...
            <div key={id} className="flex flex-col gap-2 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
              ...
              <div className="flex flex-col gap-1.5">
                {snippets.slice(0, 3).map((r, i) => (
                  <button
                    key={i}
                    onClick={() => { onOpenSession(id, r.turnUuid); handleClose() }}
